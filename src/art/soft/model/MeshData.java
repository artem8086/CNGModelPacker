...
public class MeshData {
    ...
    public void draw(Graphics g, ModelAnim anim, int x, int y, float zoom, boolean flipX, boolean flipY) {
        ModelSet model = anim.getData();
        ModelPacker packer = Loader.getLoader().packer;
        {
            int xL = ModelPacker.translateX + (packer.getWidth() >> 1);
            int yL = ModelPacker.translateY + (packer.getHeight() >> 1);
            x -= xL; y -= yL;
            // Расчет вершин
            float z;
            int start = 0;
            ...
            for (int i = 2; i > 0; i --) {
                for (; start < end; start++) {
                    ...
                    if (start == tIndx) {
                        ...
                        //
                        //System.out.println("mx = " + mx + ", my = " + my + ", z = " + z);
                        //
